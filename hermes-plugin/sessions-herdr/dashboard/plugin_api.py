"""sessions-herdr plugin API — Hop G: A3 action pack on top of Hop C create/link.
...
EVIDENCE_PORT = 8766
FLEET_BRIDGE_PORT = 8788  # not evidence — fleet-bridge; do not relabel the strip
...
def _evidence(session: SessionRef) -> dict[str, Any]:
    ...
    fleet = _probe_port(FLEET_BRIDGE_PORT)
    ...
    return {
        ...
        "reason": None if listening else f":{EVIDENCE_PORT} not listening",
        "reconcile": "8788 is fleet-bridge, not the evidence hub. Evidence link stays :8766.",
        "fleetBridgeListening": bool(fleet.get("listening")),
    }
...
def ports() -> dict[str, Any]:
    ...
    return {
        ...
        "reconcile": "8788 is fleet-bridge, not evidence. Strip stays 8642/9119/8766/9120.",
    }
...
def health() -> dict[str, Any]:
    return {"ok": True, "plugin": "sessions-herdr", "hop": "G", "actions": [
